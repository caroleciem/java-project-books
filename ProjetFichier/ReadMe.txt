Le package ProjetFichierMVP  contient le projet java-project-books composé de quatre Classes + readme:
La classe MenuProjet2 qui contient le Main et toute la partie du menu principal
la classe MenuLivre2 qui contient la gestion du menu livre (et sous-menu) lorsque le choix 4 a été effectué
la classe Mot utilisée pour faire des objets mot (avec le libelle du mot et sont nb d'occurrence)
la classe Pretraitement qui gérera le prétraitements des fichiers en créant des fichiers préfixés par t pouvant être utilisés dans le menuLivre

Il est possible de mettre des noms de fichiers en arguments pour initialiser ces fichiers dès l'affichage du menu principal. ces fichiers ne seront pas prétraités (le prétratement est intégré dans cette version
...
3 : afficher les 50 mots les plus courants
...
Choix 3 :affiche les 50 mots les plus courants avec leur nombre d'occurrences